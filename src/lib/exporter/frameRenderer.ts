...
import { layoutVideoContent } from "@/components/video-editor/videoPlayback/layoutUtils";
import { getWebcamOverlaySizePx } from "@/components/video-editor/webcamOverlay";
import { resolveMediaElementSource } from "./localMediaSource";

const CURSOR_FRAME_DISCONTINUITY_MS = 100;
...
export class FrameRenderer {
	...
	private lastCursorFrameTimeMs: number | null = null;
	...
	private async syncWebcamFrame(targetTime: number): Promise<void> {
		...
		const duration = Number.isFinite(webcamVideo.duration) ? webcamVideo.duration : targetTime;
	...
	async renderFrame(videoFrame: VideoFrame, timestamp: number): Promise<void> {
		...
		if (this.cursorOverlay) {
			const freezeCursor =
				this.lastCursorFrameTimeMs === null ||
				Math.abs(timeMs - this.lastCursorFrameTimeMs) > CURSOR_FRAME_DISCONTINUITY_MS;
			...
			this.lastCursorFrameTimeMs = timeMs;
		}
	...
	private updateLayout(): void {
		...
		const { borderRadius = 0, padding = 50 } = this.config;
		...
		const layout = layoutVideoContent({
			...
			app: {
				...
				renderer: {
					...this.app.renderer,
					resize:
						typeof this.app.renderer.resize === "function"
							? this.app.renderer.resize.bind(this.app.renderer)
							: () => undefined,
				},
				canvas: {
					...(this.app.canvas ?? {}),
					style: (this.app.canvas as HTMLCanvasElement | undefined)?.style ?? {},
				},
			} as Application,
	...
	private drawWebcamOverlay(ctx: CanvasRenderingContext2D, width: number, height: number): void {
		...
		const webcamFrameSource = canRefreshCache ? webcamVideo : this.webcamFrameCacheCanvas;
