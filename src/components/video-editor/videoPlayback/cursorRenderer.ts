		...
		cursorAssetsPromise = (async () => {
			...
			const entries = await Promise.all(
				SUPPORTED_CURSOR_KEYS.map(async (key) => {
					...
					const nativeAssetUrl = !isLinux ? systemAsset?.dataUrl : undefined;
					const assetUrl = nativeAssetUrl ?? uploadedAsset?.url ?? systemAsset?.dataUrl;
					...
					try {
						...
						if (uploadedAsset && !nativeAssetUrl) {
							...
							width =
								systemAsset?.width && systemAsset.width > 0 ? systemAsset.width : img.naturalWidth;
							height =
								systemAsset?.height && systemAsset.height > 0
									? systemAsset.height
									: img.naturalHeight;
...
function getCursorRenderTarget(samples: CursorTelemetryPoint[], timeMs: number) {
	const target = interpolateCursorPosition(samples, timeMs);
	if (!target) {
		return null;
	}

	const latestSample = findLatestSample(samples, timeMs);
	const latestClick = findLatestInteractionSample(samples, timeMs);
	const shouldSnapToClick =
		!!latestClick &&
		!!latestSample &&
		latestSample.timeMs >= latestClick.timeMs &&
		Math.abs(timeMs - latestClick.timeMs) <= CLICK_ANIMATION_MS;

	if (shouldSnapToClick) {
		return {
			cx: latestClick.cx,
			cy: latestClick.cy,
			snapToTarget: true,
...
export class PixiCursorOverlay {
	...
	update(
		...
		const target = getCursorRenderTarget(samples, timeMs);
		if (!target) {
			this.container.visible = false;
			return;
		}
		...
		if (shouldFreezeCursorMotion || target.snapToTarget) {
...
export function drawCursorOnCanvas(
	...
	if (target.snapToTarget) {
		smoothedState.snapTo(target.cx, target.cy, timeMs);
	} else {
